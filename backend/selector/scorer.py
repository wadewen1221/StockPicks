...
def evaluate_buy_signal(hist_data, stock_info):
    """
    超买超卖综合评估
    返回买卖信号
    """
    kdj_rsi_cci = calculate_kdj_rsi_cci(hist_data)
...
def evaluate_mid_long_term(stock_data, hist_data):
    """评估中长线投资价值"""
...
def evaluate_short_term(stock_data, hist_data):
    """评估超短线机会"""
...
def calculate_mid_long_score_v2(hist_data, stock_info, indicators=None, market_thresholds=None):
    """
    中长线股票优化评分模型 V2
    五大维度综合评分（满分95分）

    Args:
        market_thresholds: 市场自适应阈值 dict。需由调用方传入，
            避免函数变成隐藏的IO调用。
    """
    ...
    rsi_low = market_thresholds['rsi_oversold']
    rsi_high = market_thresholds['rsi_overbought']
    ...
    pos_low = market_thresholds['position_low']
    pos_high = market_thresholds['position_high']
    ...
    vol_thresh = market_thresholds['vol_ratio_threshold']
...
def calculate_short_term_aggressive(hist_data, stock_info):
    """
    超短线激进型评分模型
    """
    if not hist_data or len(hist_data) < 60:
...
def calculate_short_term_conservative(hist_data, stock_info):
    """
    超短线稳健型评分模型
    """
    if not hist_data or len(hist_data) < 60:
